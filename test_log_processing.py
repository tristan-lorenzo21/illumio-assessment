...
class TestLogProcessing(unittest.TestCase):
    ...
    def test_get_parsed_log(self):
        result = get_parsed_log("test_flow_log.txt", "input_files/number-protocol.csv")
        expected = [
            (49153, "tcp"),
            (49154, "tcp"),
            (49155, "tcp"),
    ...
    def test_get_tag_counts(self):
        parsed_log = get_parsed_log(
            "input_files/input.txt", "input_files/number-protocol.csv"
        )
        lookup_table = get_lookup_table("input_files/lookup_table.txt")
        result = get_tag_counts(parsed_log, lookup_table)
        expected = {"Untagged": 8, "sv_P2": 1, "sv_P1": 2, "email": 3}
        self.assertEqual(result, expected)

    def test_get_combination_counts(self):
        parsed_log = get_parsed_log(
            "test_flow_log.txt", "input_files/number-protocol.csv"
        )
        result = get_combination_counts(parsed_log)
        expected = [[49153, "tcp", 1],[49154, "tcp", 1],[49155, "tcp", 1],[49156, "tcp", 1], [49157, "tcp", 1],[49158, "tcp", 1], [80, "tcp", 1], [1024, "tcp", 1], [443, "tcp", 1], [23, "tcp", 1], [25, "tcp", 1], [110, "tcp", 1], [993, "tcp", 1], [143, "tcp", 1], ]
        self.assertEqual(result, expected)

    def test_invalid_log_file(self):
        result = get_parsed_log(
            "input_files/test_txt_file.txt", "input_files/number-protocol.csv"
        )
    ...
    def test_missing_log_file(self):
        result = get_parsed_log("missing.txt", "input_files/number-protocol.csv")
    ...
    def test_ascii_check_log(self):
        result = get_parsed_log(
            "input_files/ascii.txt", "input_files/number-protocol.csv"
        )
    ...
    def test_get_protocol_map(self):
        result = get_protocol_map("input_files/number-protocol.csv")

        expected = self.protocol_map_data
