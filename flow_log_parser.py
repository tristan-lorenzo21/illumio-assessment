...
def get_tag_counts(parsed_log, lookup_table):
    ...
    try:
        ...
        if "'NoneType' object is not iterable" in str(error):
            return (
                "Cannot iterate through the log or lookup table file - get_tag_counts"
            )
        else:
            return f"The following error occured: {error}"
...
def get_combination_counts(parsed_log):
    ...
    print(formatted_combination_counts)
...
def get_output_file(tag_counts, combination_counts):
    ...
    if len(tag_counts) < 1 or combination_counts is None:
        print(
            "The flow log or lookup table file is invalid. Please try again with valid files. - get_output_file"
        )
    else:
...
tag_counts = get_tag_counts(
    get_parsed_log(
        "input_files/input.txt", get_protocol_map("input_files/number-protocol.csv")
    ),
    get_lookup_table("input_files/lookup_table.txt"),
)
combination_counts = get_combination_counts(
    get_parsed_log(
        "input_files/input.txt", get_protocol_map("input_files/number-protocol.csv")
    )
)
...
# tag_counts = get_tag_counts(get_parsed_log('input_files/sample_log.txt', get_protocol_map('input_files/number-protocol.csv')), get_lookup_table('input_files/lookup_table.txt'))
# combination_counts = get_combination_counts(get_parsed_log('input_files/sample_log.txt', get_protocol_map('input_files/number-protocol.csv')))
# get_output_file(tag_counts, combination_counts)
